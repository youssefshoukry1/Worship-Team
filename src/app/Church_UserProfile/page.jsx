...
export default function Church_UserProfile() {
        ...
        const loadProfile = async () => {
            try {
                ...
                setProfile({
                    ...
                    prayTime: (currentUser.prayTime || []).sort((a, b) => new Date(b.date) - new Date(a.date)),
    ...
    const handleSubmitPrayTime = async () => {
        ...
        try {
            ...
            const updatedPrayTime = (data.user?.prayTime || []).sort((a, b) => new Date(b.date) - new Date(a.date));
            setProfile(prev => ({
                ...prev,
                prayTime: updatedPrayTime
            }));
            resetPrayForm();
        } catch (submitError) {
                    ...
                    {activeTab === 'overview' && (
                        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                            ...
                            <SummaryCard
                                ...
                                primaryValue={displayProfile?.prayTime?.length || 0}
                                secondaryLabel="Encouraged"
                                secondaryValue={displayProfile?.prayTime?.length || 0}
                        ...
                        <div className="grid gap-5 sm:gap-6">
                            <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-rose-500/10 via-slate-900/70 to-slate-900 p-4 sm:p-6">
                                ...
                                <p className="text-xs sm:text-sm text-slate-300 mb-4 leading-relaxed">
                                    This is your safe place with God. Pick your feeling and write honestly, one line at a time.
                                </p>
